...
- `POST /connectors/gmail/drafts`: creates a Gmail draft using JSON `{ "to": "...", "cc": [], "bcc": [], "subject": "...", "body": "...", "indexed_attachments": [{ "source_id": "...", "relative_path": "resume.pdf" }], "generated_attachments": [{ "id": "generated_...", "file_name": "black-hole-summary.md" }] }`, or multipart/form-data with fields `to`, `cc`, `bcc`, `subject`, `body`, optional `indexed_attachments` JSON, optional `generated_attachments` JSON, and repeated `attachments` files. It never sends the draft. Manual, indexed, and generated-output attachments are resolved and validated server-side; generated outputs must resolve inside the MindOS outputs directory. Blocked extensions include `.exe`, `.dll`, `.bat`, `.cmd`, `.ps1`, `.sh`, `.key`, `.pem`, `.env`, `.sqlite`, `.db`, `.zip`, `.rar`, and `.7z`, and total attachments are capped at 20 MB.
- `POST /connectors/gmail/send`: sends a Gmail message only when Gmail is connected, Gmail `send_email` capability is available, recipient/subject/body are present, and `{ "confirmation": true }` is provided. Request:
  `{ "to": ["person@example.com"], "cc": [], "bcc": [], "subject": "...", "body": "...", "confirmation": true, "generated_attachments": [{ "id": "generated_...", "file_name": "black-hole-summary.md" }] }`.
  The endpoint also accepts multipart/form-data with fields `to`, `cc`, `bcc`, `subject`, `body`, `confirmation`, optional `indexed_attachments` JSON, optional `generated_attachments` JSON, and repeated `attachments` files. Missing confirmation returns a JSON error. Attachment validation matches the draft endpoint. Gmail credentials/tokens, message bodies, and attachment contents are not stored in Memory.
...
- `POST /tasks/document/summary/save-output`: save generated markdown/text output from a connected-folder summary/report. Request:
  ...
  Response includes `{ "ok": true, "output_file": { "id": "generated_...", "file_name": "black-hole-report.md", "path": "C:\\Users\\...\\.mindos\\outputs\\black-hole-report.md", "size_bytes": 1234, "mime_type": "text/markdown", "source_task_id": "...", "source_step_id": "document.create_output_file", "selected": true } }`. Filenames are sanitized and never overwritten; collisions receive a numeric suffix. The `id` plus `file_name` can be used later as a Gmail `generated_attachments` reference.
