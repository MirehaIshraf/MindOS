...
| Gmail connector | POC | Dedicated local-first Gmail OAuth connector. User uploads their own Google OAuth desktop `credentials.json`, connects through a localhost callback, and MindOS stores the Gmail token locally under `~/.mindos/connectors/gmail/`. Supports profile test, recent metadata reads, draft creation, Gmail attachments, and Gmail send after explicit UI confirmation when Gmail capabilities allow it. Gmail task actions route through Gmail-specific status/routes, not the generic Email MCP connector. No broad Gmail full-access scope is requested. |
...
- Gmail attachment preview supports explicit manual file selection and lightweight candidate search from recent summary outputs or the currently selected browser folder scan. Attachments are shown before draft/send, dangerous file types are blocked, total attachment size is capped at 20 MB, and Gmail draft/send routes accept multipart attachments only after user action. Attachment contents are never stored in Memory or Task History.
