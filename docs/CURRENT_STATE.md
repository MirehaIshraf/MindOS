...
| Jira connector | POC | Jira Cloud connection POC supports site URL, Atlassian email, Jira API token, default project key, connection test, connect/disconnect, and project listing. Atlassian Organization Admin API keys and Organization IDs are not used for this connector. No Jira issue creation is implemented. |
...
- Jira connector POC: Jira Cloud API-token authentication with site URL, Atlassian email, and Jira API token. The connector can test `/rest/api/3/myself`, list accessible projects through `/rest/api/3/project/search`, and store a default project key/issue type for future tasks. Tokens are stored locally, never returned to the frontend, never stored in Memory, and Organization Admin API keys are not accepted.
