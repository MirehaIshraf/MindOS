...
| Jira connector | POC | Jira Cloud connection POC supports site URL, Atlassian email, Jira API token, default project key, connection test, connect/disconnect, project listing, read-only issue search, and confirmed issue creation after editable preview. Atlassian Organization Admin API keys and Organization IDs are not used. |
...
- Jira connector POC: Jira Cloud API-token authentication with site URL, Atlassian email, and Jira API token. The connector can test `/rest/api/3/myself`, list accessible projects through `/rest/api/3/project/search`, search existing issues with safe JQL, and create a Jira issue only after an editable preview plus explicit confirmation. Tokens are stored locally, never returned to the frontend, never stored in Memory, and Organization Admin API keys are not accepted.
...
Jira is a real POC connector for connection, project metadata, issue search, and confirmation-gated issue creation. GitHub and Email MCP remain read-only POC connectors.
...
- A unified action approval/execution foundation now exists for Gmail, Jira, future GitHub, and local Git actions. Prepared actions use a capability registry, render a preview, require explicit confirmation for risky side effects, and record user-facing Task History. Gmail task actions now use the Gmail connector directly for search previews, draft creation, and send previews. Jira task actions search existing issues first, show editable issue previews, and create only after confirmation. Sending always requires Gmail `send_email`, valid recipient/subject/body fields, and explicit confirmation; disabled buttons show connector-specific reasons. GitHub write and local Git write actions remain disabled.
