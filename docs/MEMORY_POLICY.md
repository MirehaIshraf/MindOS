...
| file_system/file_indexed | Yes | Yes | Yes | Yes | Connected-folder readable content index. Created only when readable text extraction succeeds. Stores relative path, metadata, and bounded excerpt only; secrets/blocked files are skipped. |
...
| email/email_message | Yes | Yes | Yes | Yes | Read-only Email MCP sync. Stores normalized message metadata and excerpt only. No full body, no attachment contents, no credentials. |
...
- File System connected-folder inventory is operational connector metadata, not Memory. It may include safe file names, relative paths, extensions, size, modified date, attachability, missing status, and content indexing status for files under user-connected folders. It exists so filename search and Gmail attachment discovery can find files even when content extraction fails.
- File System connected-folder `file_indexed` events are visible, indexable, relationship eligible, and context eligible. They must come only from user-saved/tracked folder sources, store relative paths plus bounded excerpts, and be created only after readable content extraction succeeds. Failed content extraction should update inventory status, not create noisy Memory events. Index lifecycle activity such as source save/update, reindex queued, index job started/completed/failed, and stale-source polling must not create Memory events. Missing files are marked in existing `file_indexed` metadata rather than deleting the memory record.
...
- Gmail connector OAuth setup, profile tests, draft preparation, draft creation, and draft sending are not Memory events. Gmail client secrets, access tokens, refresh tokens, authorization codes, draft bodies, and attachment contents must never be stored in Memory or indexed by default. Gmail draft/send activity may appear in local Task History with lightweight title/status and attachment filenames only.
- Gmail attachment candidate search may use connected File System inventory and content indexes, but selected attachment contents are resolved only for the confirmed Gmail request. Attachment bytes and full paths are not Memory events and must not be indexed from Gmail activity.
- Generated summary/report output files may be referenced in Gmail task previews and Task History by generated id, filename, size, and MIME type only. Generated output contents, source file contents, and full Gmail bodies must not be stored in Memory or Task History.
- Gmail synced/read email metadata or snippets may become Memory only through an explicit sync/read policy. Gmail task draft/send previews, bodies, and attachment contents are not Memory.
- Email MCP connector `email_message` events are visible, indexable, relationship eligible, and context eligible. They store subject, sender, recipients, date, labels/folder, URL, attachment metadata, and body excerpt only. Email API keys/tokens, provider config secrets, full email bodies, and attachment contents are never stored in Memory. Email connector credential save, connection test, capability discovery, folder listing, sync start, and sync completion are not Memory events.
...
- Clearing Task History removes only local task-history records and the browser-local Recent Tasks list. It must not delete Memory events, connector credentials/data, Gmail/email data, indexed file records, chats, settings, or files.
- Unified task action execution writes user-facing Task History records for completed/failed side-effect actions. Gmail draft/send/reply-draft task results, Git commit/push task results, and GitHub write task results are Task History only unless a later explicit policy says otherwise. They must not store full email bodies, secrets, or raw provider credentials in Memory. Read-only email search previews do not create Memory events unless the user runs the explicit email sync/import flow.
- GitHub read sync events are Memory. Future GitHub write task results such as created issues or pull requests are Task History only unless explicitly reclassified later.
- Jira issue search results and Jira issue creation tasks are Task History only in this POC. They must not create Memory events or indexing entries. Future Jira sync/import may create read-only Jira memory events under a separate explicit policy.
- Future local Git commit/push task results are Task History only by default. Do not index operational task execution logs, command output, or push/commit activity as Memory unless an explicit memory policy says otherwise.
...
- Connected-folder search summary/report tasks may read selected connected File System files to generate a new output file, but source document text and full generated report content must not be stored in Task History or Memory. Completion may create the usual lightweight `document_summary_created` event with output filename, topic, counts, and source folder metadata.
- Log analysis report tasks may read selected connected `.log`/`.txt` files, but raw logs, redacted evidence excerpts, and full generated reports must not be stored in Memory or indexed by default. Log report completion may appear in local Task History with lightweight counts, filenames, output filename, and model metadata only.
