...
- CommandPlans and Run History are operational records, not Memory by default. They may store sanitized plan summaries, statuses, confirmation state, lightweight tool result summaries, and user-facing error messages, but they must not be embedded/indexed unless a future explicit policy says a specific result type should become Memory.
- Tool outputs stored in command history must be sanitized. Credentials, tokens, OAuth codes, API keys, raw provider secrets, full email bodies, attachment bytes, and raw file contents must never be stored in Memory. Full email bodies, full attachments, and raw connected-file contents should not be stored in Run History either.
- Connector read-sync Memory is separate from command execution history. GitHub/Jira read connector imports may create indexable source events when explicitly synced/imported. Chat command runs that create/update/comment/send/write should remain Run History only unless a later explicit policy reclassifies a small sanitized result.
