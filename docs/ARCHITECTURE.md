...
LLM-first Task Planning flow: instruction -> deterministic pre-parser extracts hints such as emails, file terms, topic terms, explicit filenames, extensions, action words, connector words, attachment/output/risk words, date hints, and possible intents -> selected-model planner returns structured JSON -> backend validates allowed transitions, repairs obvious misroutes, blocks low-confidence/unsafe plans, and normalizes steps -> frontend renders the first required preview step -> each side-effect step still requires user selection/confirmation. Deterministic fallback is used only when the model is unavailable, times out, or returns invalid/unsafe output. File-dependent Gmail tasks use `file.search_connected_folders` before Gmail preview, document summary/report tasks search connected folders before reading selected files, and log-analysis tasks search connected log-like files before report generation.
...
Log Analysis Report flow: user command -> LLM-first task planner detects `log.analyzeFromSearch` -> `/tasks/files/search` searches only connected indexed folders for readable `.log`, `.txt`, `.out`, and `.err` candidates, optionally preferring latest files -> frontend filters `.txt` results to log-like filenames/content -> user selects log files -> `POST /tasks/logs/analyze/prepare` resolves files inside their connected roots, reads bounded excerpts, redacts secrets, extracts error evidence, and asks the selected model for a markdown report with deterministic fallback -> preview -> user saves through `POST /tasks/logs/analyze/save` to the local MindOS outputs folder. Original logs are never modified, raw logs are not indexed, and saved report/task history metadata stays lightweight.
